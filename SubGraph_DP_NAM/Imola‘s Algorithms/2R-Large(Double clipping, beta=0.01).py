...
def graph_projection(adj_list, epsilon_0, alpha):
    ...
    for i in range(n):
        ...
        # Because if the d_tilde[i]==0, the function calculate_kappa will occur err
        d_tilde[i] = 1 + max(alpha-1 + degrees[i] + np.random.laplace(0, 1/epsilon_0), 0)
...
def calculate_kappa(tilde_d_i, miu, beta):
    def equation(kappa):
        p1=kappa/tilde_d_i
        term = -tilde_d_i*(p1*np.log(p1/miu)+(1-p1)*np.log((1-p1)/(1-miu)))
        return np.exp(term) - beta
    #avoid p1=0
    kappa_initial_guess = max(miu*tilde_d_i,tilde_d_i-1)
    kappa_solution = fsolve(equation, kappa_initial_guess)
    kappa_solution = min(max(kappa_solution, miu*tilde_d_i), tilde_d_i)
    return kappa_solution[0]
...
def jacob(matrix, miu, eps1, eps2, tilde_d, beta):
    ans=0
    n = matrix.shape[0]
    ...
    for i in range(n):
        ...
        kappa=calculate_kappa(tilde_d[i], miu, beta)

        hat_wi=wi+np.random.laplace(loc=0, scale=kappa/eps2)
...
for eps in epsilon_list:
    epsilon_0, epsilon_1, epsilon_2 = eps * 0.1, eps * 0.8, eps * 0.1
    print("epsilon:",eps,":")
    ...
    for k in range(20):
        ...
        guss_tri = jacob(B, miu, epsilon_1, epsilon_2, tilde_d, beta)
        print(guss_tri)
