...
def jacob(matrix, miu, eps1, eps2, tilde_d):
    ...
    for i in range(n):
        ...
        ans+=wi + np.random.laplace(loc=0, scale=tilde_d[i] / eps2)
...
for eps in epsilon_list:
    ...
    for k in range(20):
        ...
        # GraphProjection
        prime_adj_list, tilde_d = graph_projection(adjacency_list, 1/8*eps1, alpha)

        B = RR_clipping(adjacency_list,eps1)

        guss_tri = jacob(B, miu, eps1, 0.1*eps, tilde_d)
        print(guss_tri)
