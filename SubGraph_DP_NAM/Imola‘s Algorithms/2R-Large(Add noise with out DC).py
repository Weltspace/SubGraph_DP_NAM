...
def jacob(matrix, miu, eps1, eps2, tilde_d):
    ...
    for i in range(n):
        ...
        hat_wi=wi+np.random.laplace(loc=0, scale=tilde_d[i]/eps2)
...
for eps in epsilon_list:
    ...
    for k in range(20):
        ...
        prime_adj_list=adjacency_list

        degrees = [0] * len(adjacency_list)
        for i, neighbors in enumerate(adjacency_list):
            degrees[i] = len(neighbors)

        B = RR_clipping(prime_adj_list,epsilon_1)

        guss_tri = jacob(B, miu, epsilon_1, epsilon_2, degrees)
        print(guss_tri)
