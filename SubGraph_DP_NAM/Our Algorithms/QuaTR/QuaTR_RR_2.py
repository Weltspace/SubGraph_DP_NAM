...
for eps in epsilon_list:
    ...
    for k in range(rounds):
        ...
        for node in range(0, n):
            # set the Delt_f
            Delt_f = norm.ppf(1 - beta) * np.sqrt( tilde_d[node] * ((n - 1) * sigma ** 4 + 2 * tilde_d_max* sigma ** 2 ))  + (tilde_d_max-1)*tilde_d[node]
            ...
            for i in range(d_i):
                small_sum = 0
                for j in range(d_i):
                    small_sum += hat_B_RR[neighbors[i], neighbors[j]]
                total_sum += min(max(small_sum, -Delt_f), Delt_f)
