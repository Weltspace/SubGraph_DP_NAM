...
for eps in epsilon_list:
    ...
    for k in range(rounds):
        ...
        for node in range(0, n):
            # set the Delt_f
            Delt_f = norm.ppf(1 - beta) * np.sqrt( (n - 1) * sigma ** 4 + (tilde_d[node] + tilde_d_max) * sigma ** 2) + tilde_d[node]
            ...
            for j in adj_list[node]:
                if j != node:
                    total_sum += min(max(hat_B_RR[j,node], -Delt_f), Delt_f)
            total_sum += np.random.laplace(loc=0, scale=Delt_f / epsilon_2)

        guss_tri_RR = total_sum / 6

        print("TriMTR_RR:", guss_tri_RR)
    ...
    print('\n')
    print('epsilon=',eps,':')
    print("TriMTR_RR:",[float(x) for x in list_RR] ,', MSE:', MSE_RR, '     MRE:', ReE_RR)
    print('\n')
